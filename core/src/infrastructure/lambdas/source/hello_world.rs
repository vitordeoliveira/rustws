//! Hello World Lambda Function - Using #[lambda_fn] attribute macro with HTTP and full IDE support
//!
//! This demonstrates:
//! - Using the enhanced #[lambda_fn] attribute macro with features = [http]
//! - Making HTTP requests from WASM lambdas (auto-generated by macro)
//! - Automatic JSON schema generation for workflow validation
//! - Pure business logic with FULL IDE AUTOCOMPLETE SUPPORT!
...
fn hello_world_lambda(input: HelloWorld) -> HelloWorld {
    ...
    // Validation - IDE knows input is HelloWorld!
    if input.count > 50 {
        panic!("Count is too high");
    }

    // Business logic with HTTP request - pure Rust with full IDE support!
...
//
// This lambda uses the new #[lambda_fn(features = [http])] attribute macro.
...
// ✅ Testability: You can call hello_world_lambda() directly for testing!
// ✅ Clean syntax: Looks like a normal Rust function
//
// Comparison:
// OLD: lambda_fn! { features: [http], handler: |input: Type| -> Type { ... } }
// NEW: #[lambda_fn(features = [http])] fn my_lambda(input: Type) -> Type { ... }
//
...
//   let test_input = HelloWorld { text: "test".to_string(), count: 5, post: None };
//   let result = hello_world_lambda(test_input);  // Can call directly!
